...
export class ClipService {
  ...
  async clipRecording(recordingName, startTime, endTime, time) {
    ...
    try {
      ...
      return {
        success: true,
        clipUrl: await this.getClipUrl(clipName),
        clipName: clipName,
      };
    } catch (error) {
  ...
  async saveClipRecording(title, clipName) {
    try {
      ...
      const clipUrl = await this.getClipUrl(this.getClipKey(clipName));
      const sql =
  ...
  async saveClipThumbnail(recordingName, tempThumbnailPath) {
    ...
    try {
      ...
      // S3에 업로드
      await s3Service.uploadImage(thumbnailKey, imageBuffer);

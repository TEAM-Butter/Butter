...
function CrewDetailPage() {
  ...
  let [crewScheduleDetail, setCrewScheduleDetail] = useState([
    "1번",
    "2번",
    "3번",
  ]);
  let [crewNoticeDetail, setCrewNoticeDetail] = useState([
    "1번 공지사항",
    "2번 공지사항",
    "3번 공지사항",
  ]);
    ...
    <PageContainer>
      ...
      <LayOut2>
        <ScheduleEditComponent
          crewScheduleDetail={crewScheduleDetail}
          crewDetail={crewDetail}
        />
        ...
        <Box7>
          {" "}
          {crewNoticeDetail.map((a, i) => {
            return (
              <div key={i}>
                {i + 1}번 공지사항 - 내용 : {a}{" "}
                <button
                  onClick={() => {
                    navigate(`/crew/notice/detail/${id}/${i}`);
                  }}
                >
                  공지사항 자세히보기
                </button>
  ...
  return (
    <div>
      <div>
        ...
        <div>크루 설명 : {crewDetail}</div>
        <div>
          크루 멤버1 사진 : {crewDetail} <button>멤버삭제</button>
        </div>
        <div>
          크루 멤버2 사진 : {crewDetail} <button>멤버삭제</button>
        </div>
        <div>
          크루 멤버3 사진 : {crewDetail} <button>멤버삭제</button>
        </div>
        <div>
          크루 멤버4 사진 : {crewDetail} <button>멤버삭제</button>
        </div>

        <button onClick={() => setCrewMemberPlusModalOpen(true)}>
          멤버추가
        </button>
      </div>
      <div>
      ...
      {crewMemberPlusModalOpen && (
        <CrewMemberEditModalOverlay>
          <CrewMemberEditModalContent>
            ...
            <div>
              <button onClick={() => setCrewMemberPlusModalOpen(false)}>
                닫기
              </button>
            </div>
          </CrewMemberEditModalContent>
      ...
      {isScheduleDetailModalOpen && (
        <ScheduleDetailModalOverlay>
          <ScheduleDetailModalContent>
            ...
            <div>
              {" "}
              스케쥴 제목 : {crewScheduleDetail[selectedScheduleIndex - 1]}
            </div>
